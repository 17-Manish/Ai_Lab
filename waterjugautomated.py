...
a=int(input("enter jug A capacitiy:"))
b=int(input("enter jug B capacitiy:"))
...
if ai<0 or bi<0 or af<0 or bf<0:
    print("negative values must are not allowed.")
    exit(1)
if ai==af and bi==bf :
...
def bfs_wjug(a,b,ai,bi,af,bf):
    ...
    while queue:
        ...
        possible_operations = [
            (a,curr_bi,"fill jug A"),(curr_ai,b, "fill jug B"),(0,curr_bi, "empty jug A"),(curr_ai,0,"empty jug B"),
            (curr_ai - min(curr_ai,b-curr_bi),
             curr_bi + min(curr_ai,b-curr_bi),"pour from A to B"),
